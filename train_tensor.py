...
parser.add_argument('--num-rank', type=int, default=200, help='')
...
def main():
    ...
    factor_path = f"./factor_{args.num_rank}.pkl"
    with open(f"{factor_path}", "rb") as f:
    ...
    engine = MDN_trainer(scaler, args.in_dim, args.seq_length, args.num_nodes, args.num_rank, args.nhid, args.dropout,
                         ...
                         mse_coef=args.mse_coef, nonlinearity=args.nonlinearity, loss=args.loss, factor=factor)
    ...
    for i in range(args.epochs):
        ...
        for iter, (x, y) in enumerate(dataloader['train_loader'].get_iterator()):
            ...
            train_mu_reg_loss.append(metrics['mu_reg'])
        ...
        for iter, (x, y) in enumerate(dataloader['val_loader'].get_iterator()):
            ...
            valid_mu_reg_loss.append(metrics['mu_reg'])
        ...
        for iter, (x, y) in enumerate(dataloader['test_loader'].get_iterator()):
            ...
            test_mu_reg_loss.append(metrics['mu_reg'])
        ...
        # mtrain_mae2 = np.stack(train_mae2, 0).mean(0)
        # mtrain_mape2 = np.stack(train_mape2, 0).mean(0)
        # mtrain_rmse2 = np.stack(train_rmse2, 0).mean(0)
        mtrain_res_loss = np.mean(train_res_loss)
        mtrain_mse_loss = np.mean(train_mse_loss)
        mtrain_mu_reg_loss = np.mean(train_mu_reg_loss)

        mvalid_loss = np.mean(valid_loss)
        mvalid_mae = np.mean(valid_mae)
        mvalid_mape = np.mean(valid_mape)
        mvalid_rmse = np.mean(valid_rmse)
        # mvalid_mae2 = np.stack(valid_mae2, 0).mean(0)
        # mvalid_mape2 = np.stack(valid_mape2, 0).mean(0)
        # mvalid_rmse2 = np.stack(valid_rmse2, 0).mean(0)
        mvalid_res_loss = np.mean(valid_res_loss)
        mvalid_mse_loss = np.mean(valid_mse_loss)
        mvalid_mu_reg_loss = np.mean(valid_mu_reg_loss)
        ...
        mtest_mae2 = np.stack(test_mae2).mean(0)
        mtest_mape2 = np.stack(test_mape2).mean(0)
        mtest_rmse2 = np.stack(test_rmse2).mean(0)
        mtest_res_loss = np.mean(test_res_loss)
        mtest_mse_loss = np.mean(test_mse_loss)
        mtest_mu_reg_loss = np.mean(test_mu_reg_loss)
        ...
        for p0 in args.pred_len:
            # engine.summary.add_scalar(f'errors_spec/train_mae_{}', mtrain_mae2, i)
            # engine.summary.add_scalar(f'errors_spec/train_mape', mtrain_mape2, i)
            # engine.summary.add_scalar(f'errors_spec/train_rmse', mtrain_rmse2, i)
            # engine.summary.add_scalar(f'errors_spec/val_mae', mvalid_mae2, i)
            # engine.summary.add_scalar(f'errors_spec/val_mape', mvalid_mape2, i)
            # engine.summary.add_scalar(f'errors_spec/val_rmse', mvalid_rmse2, i)
            engine.summary.add_scalar(f'errors_spec/test_mae_{p0}', mtest_mae2[p0], i)
            engine.summary.add_scalar(f'errors_spec/test_mape_{p0}', mtest_mape2[p0], i)
            engine.summary.add_scalar(f'errors_spec/test_rmse_{p0}', mtest_rmse2[p0], i)
        ...
        engine.summary.add_scalar('loss/train_mse_loss', mtrain_mse_loss, i)
        engine.summary.add_scalar('loss/val_mse_loss', mvalid_mse_loss, i)
        engine.summary.add_scalar('loss/test_mse_loss', mtest_mse_loss, i)
        engine.summary.add_scalar('loss/train_mu_reg_loss', mtrain_mu_reg_loss, i)
        engine.summary.add_scalar('loss/val_mu_reg_loss', mvalid_mu_reg_loss, i)
        engine.summary.add_scalar('loss/test_mu_reg_loss', mtest_mu_reg_loss, i)
