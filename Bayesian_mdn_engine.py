...
class LowRankMDN(nn.Module):
    def __init__(self, n_components, n_vars, n_rank, pred_len=12):
        ...
        self.D = nn.Parameter(torch.randn(
            n_components, n_vars), requires_grad=True)
        self.V = nn.Parameter(torch.randn(
            n_components, n_vars, n_rank), requires_grad=True)
...
class MDN_trainer():
    def __init__(self, scaler, in_dim, seq_length, num_nodes, num_rank, nhid, dropout, lrate, wdecay, device, supports, gcn_bool, addaptadj, aptinit, n_components):
        ...
        self.mdn = LowRankMDN(n_components, num_nodes, num_rank)
        ...
        self.optimizer = optim.Adam(
            list(self.model.parameters()) + list(self.mdn.parameters()) + list(self.fc_w.parameters()), lr=lrate, weight_decay=wdecay)
    ...
    def train(self, input, real_val):
        ...
        mus = (self.mdn.mu).unsqueeze(0).repeat(input.size(0), 1, 1)
        D = nn.functional.elu(self.mdn.D) + 1
        D = D.unsqueeze(0).repeat(input.size(0), 1, 1)
        V = (self.mdn.V).unsqueeze(0).repeat(input.size(0), 1, 1, 1)
    ...
    def eval(self, input, real_val):
        ...
        if self.specific:
            self.specific_eval(
                features={'w': w, 'mu': mus, 'D': D, 'V': V}, y=real_val)
    ...
    def specific_eval(self, features, y):
        ...
        output = self.mdn_head.sample(
            features={'w': w, 'mu': mu, 'D': D, 'V': V}, n=1000)
