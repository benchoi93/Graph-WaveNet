...
class MDN_trainer():
    def __init__(self, scaler, in_dim, seq_length, num_nodes, num_rank, nhid, dropout, lrate, wdecay, device, supports, gcn_bool, addaptadj, aptinit, n_components, reg_coef,
        ...
        self.covariance = FixedMDN(n_components, num_nodes * self.num_pred, rho=rho, diag=diag)
    ...
    def train(self, input, real_val, eval=False):
        ...
        mus = output[:, :, :, :self.num_pred]
        mus = mus.reshape(mus.shape[0], self.n_components, self.num_nodes * self.num_pred)
