...
class FixedResCov(nn.Module):
    def __init__(self, n_components, n_vars, num_pred, num_rank, diag=False, trainL=True, device='cpu'):
...
class CholeskyResHead(nn.Module):
    ...
    def forward(self, features):
        ...
        # cp_residual = torch.stack([tl.cp_tensor.cp_to_tensor((None, [R[:, i, :], L_spatial[i], L_temporal[i]])) for i in range(c)], 1)
        # cp_residual = torch.stack([(torch.einsum("bi,cj->bcij", [L_spatial[i], L_temporal[i]]).unsqueeze(0) * R[:,i,:,:].unsqueeze(1).unsqueeze(1)).sum((-1,-2)) for i in range(c)],1)
        # cp_residual2 = [[[(i,j,k) for k in range(r)]for j in range(r)] for i in range(c)]

        # i=0
        # temp = [[torch.einsum("b,n,t->bnt" , R[:,i,j,k] , L_spatial[i,:,j] , L_temporal[i,:,k]) for k in range(r)]for j in range(r)]

        cp_residual = torch.zeros((b, n, t), device=R.device)
        for i in range(c):
            for j in range(r):
                for k in range(r):
                    cp_residual += torch.einsum("b,n,t->bnt", R[:, i, j, k], L_spatial[i, :, j], L_temporal[i, :, k])

        pred = mu + cp_residual

        mask = features['mask']

        mse_loss = torch.norm((target - mu - cp_residual) * (~mask), p=1) / (~mask).sum()
        # mse_loss = torch.norm((target - mu) * (~mask), p=1)

        reg_loss = torch.norm(R, p=1)/(~torch.isnan(R)).sum() + torch.norm(L_spatial, p=1)/(~torch.isnan(L_spatial)).sum() + torch.norm(L_temporal, p=1)/(~torch.isnan(L_temporal)).sum()
        # loss = self.rho * nll_loss + reg_loss * self.reg_coef + self.mse_coef * mse_loss
        loss = rho * reg_loss + (1-rho) * mse_loss
        # loss = mse_loss

        return loss, reg_loss.item(), mse_loss.item(), pred
...
class MDN_trainer():
    def __init__(self, scaler, in_dim, seq_length, num_nodes, num_rank, nhid, dropout, lrate, wdecay, device, supports, gcn_bool, addaptadj, aptinit, n_components, reg_coef,
        ...
        self.covariance = FixedResCov(n_components, num_nodes, self.num_pred, self.num_rank, diag=diag, trainL=rho != 0)
        ...
        self.logdir = f'./logs/GWNMDN_tensor_{datetime.datetime.now().strftime("%Y%m%d-%H%M%S")}{loss}loss_N{n_components}_R{num_rank}_reg{reg_coef}_nhid{nhid}_pred{self.num_pred}_rho{rho}_diag{diag}_msecoef{mse_coef}_nlin{nonlinearity}'
    ...
    def train(self, input, real_val, eval=False):
        ...
        fc_in = fc_in.reshape(-1, self.n_components, self.num_nodes * self.num_pred * self.num_rank)
        R = self.fc_out(fc_in)
        R = R.reshape(-1, self.n_components, self.num_rank, self.num_rank)
        ...
        loss, res_loss, mse_loss, pred = self.res_head.forward(
            features=features
        )
        ...
        features["mape"] = mape * 100
        features["rmse"] = rmse
        features["mae"] = mae
        features["mape2"] = mape2 * 100
        features["rmse2"] = rmse2
        features["mae2"] = mae2
